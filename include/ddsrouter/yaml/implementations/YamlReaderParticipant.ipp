...
/**
 * @file YamlReaderParticipant.ipp
 *
 */

#ifndef _DDSROUTER_YAML_YAMLREADERPARTICIPANT_IPP_
#define _DDSROUTER_YAML_YAMLREADERPARTICIPANT_IPP_
...
template <YamlReaderVersion V>
configuration::ParticipantConfiguration YamlReader::get<configuration::ParticipantConfiguration, V>(
        const Yaml& yml)
{
    // Id required
    ParticipantId id = get<ParticipantId, V>(yml, PARTICIPANT_NAME_TAG);

    // Kind required
    ParticipantKind kind = get<ParticipantKind, V>(yml, PARTICIPANT_KIND_TAG);

    return configuration::ParticipantConfiguration(id, kind);
}

template <YamlReaderVersion V>
configuration::SimpleParticipantConfiguration YamlReader::get<configuration::SimpleParticipantConfiguration, V>(
        const Yaml& yml)
{
    // Id required
    ParticipantId id = get<ParticipantId, V>(yml, PARTICIPANT_NAME_TAG);
    ...
    // Domain required
    DomainId domain = get<DomainId, V>(yml, DOMAIN_ID_TAG);

    return configuration::SimpleParticipantConfiguration(id, kind, domain);
}

template <YamlReaderVersion V>
configuration::DiscoveryServerParticipantConfiguration YamlReader::get<configuration::DiscoveryServerParticipantConfiguration, V>(
        const Yaml& yml)
{
    // Id required
    ParticipantId id = get<ParticipantId, V>(yml, PARTICIPANT_NAME_TAG);
    ...
    GuidPrefix guid = get<GuidPrefix, V>(yml, DISCOVERY_SERVER_GUID_PREFIX_TAG);
    ...
    if (has_domain)
    {
        domain = get<DomainId, V>(yml, DOMAIN_ID_TAG);
    }
    ...
    {
        listening_addresses = get_set<Address, V>(yml, LISTENING_ADDRESSES_TAG);
    }
    ...
    {
        connection_addresses = get_set<DiscoveryServerConnectionAddress, V>(yml, CONNECTION_ADDRESSES_TAG);
    }
    ...
    if (has_tls)
    {
        tls = get<std::shared_ptr<security::TlsConfiguration>, V>(yml, TLS_TAG);
    }
